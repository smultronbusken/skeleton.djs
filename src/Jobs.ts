...
export default class JobRegister {
    ...
    onRegister<T extends Job>(jobClass: TConstructable<T>, func: (job: T) => void) {
        let className = Reflect.getMetadata(JobRegister.metadataKey, (<TConstructable<Job>>jobClass).prototype)
    ...
    registerJob(job: Job) {
        ...
        if (jobType === undefined) {
            throw new Error(job.name + " has no info about its type. Did you forget to put the JobClass decorator on its class?")
        }
    ...
    private async getDefaultConstructors(): Promise<Job[]> {
        let messagecommand = await this.importConstructor("../data/jobs/dev_commands/messagecommand.job");
    ...
    */
     async importConstructorsInFolder(searchString: string, fgConfig: any  = {}, importPrefix: string = ""): Promise<Array<Job>> {
