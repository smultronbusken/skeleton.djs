...
import JobRegister, { CommandBase, CommandOption, CommandOptionType, CommandType, MessageCommand, SlashCommand, SlashCommandBase, SubCommand, UserCommand } from "./Jobs";
...
export default class Skeleton<T> {
  ...
  constructor(public app: T, private token: string, private clientId: string, clientOptions: ClientOptions, private guildId?: Snowflake) {
    ...
    this.client.on('interactionCreate', async interaction => {
      ...
      if (interaction.isCommand()) {
        ...
        if (subCommandName) {
          let subCommand = this.subCommands.find(subCommand => subCommand.masterCommand === interaction.commandName && subCommand.name === subCommandName)
          if (subCommand)
            subCommand.execute(interaction, app)
        } else {
  ...
  async setUpSlashCommands() {
    ...
    let createOption = <T extends SlashCommandOptionBase>(optionBuilder: T, option:CommandOption): T => {
    ...
    let addOptions = <K extends SlashCommandSubcommandBuilder | SlashCommandBuilder>(command: SlashCommandBase<any>, commandBuilder: K) =>  {
        ...
        options.forEach(option => {
          switch(option.type) {
            ...
            case CommandOptionType.Mentionable:
              commandBuilder.addMentionableOption(optionBuilder => createOption(optionBuilder, option))
              break;
    ...
    this.commands.forEach(command => {
      ...
      let subCommands = this.subCommands.filter(subCommand => subCommand.masterCommand == command.name)
